  ...
  public:
    ...
    struct fhiclConfig {
      ...
      fhicl::Atom<art::InputTag> SimChannelModuleLabel{
        ...
        G4ModuleLabel()}; // -- D.R. label not required, if not provided defaults to the value of G4ModuleLabel
      fhicl::Atom<art::InputTag> DefaultHitModuleLabel{
        fhicl::Name("DefaultHitModuleLabel"),
        fhicl::Comment("The label  of the module used to produce the hits in the art file we will "
                       "default to when no hitlist is provided."),
        "hitfd"};
    ...
    //-----------------------------------------------------
    template <typename Evt>
    const std::vector<art::Ptr<recob::Hit>> SpacePointToHits_Ps(
    ...
    HitToTrackIDEs(art::Ptr<recob::Hit> const& hit) const
    {
      return this->HitToTrackIDEs(*hit);
    }
    ...
    std::vector<sim::TrackIDE>
    HitToEveTrackIDEs(art::Ptr<recob::Hit> const& hit) const
    {
      return this->HitToEveTrackIDEs(*hit);
    }
    ...
    std::vector<sim::IDE>
    HitToAvgSimIDEs(art::Ptr<recob::Hit> hit) const
    {
      return this->HitToAvgSimIDEs(*hit);
    }

    std::vector<const sim::IDE*> HitToSimIDEs_Ps(recob::Hit const& hit) const;
    std::vector<const sim::IDE*>
    HitToSimIDEs_Ps(art::Ptr<recob::Hit> const& hit) const
    {
      return this->HitToSimIDEs_Ps(*hit);
    }
    ...
    std::vector<double>
    HitToXYZ(art::Ptr<recob::Hit> const& hit) const
    {
      return this->HitToXYZ(*hit);
    }
