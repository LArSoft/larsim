...
namespace cheat {
  ...
  //--------------------------------------------------------------------
  template <
    typename Evt> //DO NOT USE THIS FUNCTION FROM WITHIN ART! The ParticleInventoryService is designed to impliment these methods as cleanly as possible within the art framework.
  void ParticleInventory::PrepEvent(const Evt& evt)
  {
    if (!(this->CanRun(evt))) {
      throw cet::exception("ParticleInventory") << "Particle Inventory cannot function. "
  ...
  void ParticleInventory::PrepParticleList(const Evt& evt) const
  {
    ...
    for (const auto& partIn : partVecIn) {
      fParticleList.Add(new simb::MCParticle(
        partIn)); //Is this still doing a copy? If so, another method should be used.
    }
    if (fEveIdCalculator == "EmEveIdCalculator") {
      ...
      throw cet::exception("ParticleInventory3")
        ...
        << " is not a known EveIdCalculator.\n";
    }
  }
  ...
  void ParticleInventory::PrepMCTruthListAndTrackIdToMCTruthIndex(const Evt& evt) const
  {
    ...
    if (evt.getByLabel(fG4ModuleLabel, mcpmctAssnsHandle)) { // Product fetch successful
      for (const auto& mcpmctAssnIn :
           *mcpmctAssnsHandle) { //Assns are themselves a container. Loop over entries.
      ...
      if (evt.getByLabel(fG4ModuleLabel, mcpmctAssnsHandle)) { // Product fetch successful
        for (const auto& mcpmctAssnIn :
          ...
          if (mctruth_idx == USHRT_MAX) {
            fMCTObj.fMCTruthList.push_back(mct);
            fMCTObj.fTrackIdToMCTruthIndex.emplace(part->TrackId(),
                                                   fMCTObj.fMCTruthList.size() - 1);
          }
          else {
            fMCTObj.fTrackIdToMCTruthIndex.emplace(part->TrackId(), mctruth_idx);
          }
  ...
  {
    if (this->MCTruthListReady()) {
      return;
    } //If the event is data or if the truth list is already built there is nothing for us to do.
    PrepMCTruthListAndTrackIdToMCTruthIndex(
      evt); //TrackIdToMCTruthIndex and MCTruthList are prepared at the same time. The access of information makes this the most convenient way to do so. It is only somewhat more expensive for the memory, but significantly less expensive for time.
  }
  ...
  {
    if (fOverrideRealData) { return true; }
