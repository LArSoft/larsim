...
namespace cheat {
  ...
  //----------------------------------------------------------------------
  ParticleInventoryService::ParticleInventoryService(const ParticleInventoryServiceConfig& config,
  ...
  //----------------------------------------------------------------------
  ParticleInventoryService::ParticleInventoryService(const fhicl::ParameterSet& pSet,
                                                     art::ActivityRegistry& reg)
  ...
  void ParticleInventoryService::priv_PrepParticleList(const art::Event& evt)
  {
    ...
    try {
      ParticleInventory::PrepParticleList(evt);
    }
    catch (...) {
      mf::LogWarning("ParticleInventory")
        << "Rebuild failed to get the MCParticles. This is expected when running on a generation "
           "or simulation step.";
    }
  }
  ...
  void ParticleInventoryService::priv_PrepTrackIdToMCTruthIndex(const art::Event& evt)
  {
    ...
    try {
      ParticleInventory::PrepTrackIdToMCTruthIndex(evt);
    }
    catch (...) {
      mf::LogWarning("ParticleInventory")
        << "Rebuild failed to get the MCParticles. This is expected when running on a generation "
  ...
  const art::Ptr<simb::MCTruth>& ParticleInventoryService::ParticleToMCTruth_P(
  ...
  std::vector<const simb::MCParticle*> ParticleInventoryService::MCTruthToParticles_Ps(
