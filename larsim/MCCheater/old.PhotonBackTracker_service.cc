...
namespace cheat {
  ...
  //----------------------------------------------------------------------
  PhotonBackTracker::PhotonBackTracker(const fhicl::ParameterSet& pset, art::ActivityRegistry& reg)
  {
    reconfigure(pset);
  ...
  const void PhotonBackTracker::shouldThisFail() const
  {
    ...
    if (have_complained == true) {
      throw cet::exception("PhotonBackTracker1")
        << "PhotonBackTracker methods called on a file without OpDetPhotonBacktrackerRecords. "
           "Backtracked information is not available.";
    }
  }
  ...
  //----------------------------------------------------------------------
  const art::Ptr<simb::MCTruth>& PhotonBackTracker::ParticleToMCTruth(
  ...
  //----------------------------------------------------------------------
  std::vector<const simb::MCParticle*> PhotonBackTracker::MCTruthToParticles(
  ...
  std::vector<sim::TrackSDP> PhotonBackTracker::OpHitToTrackSDPs(
    art::Ptr<recob::OpHit> const& opHit)
  {
    shouldThisFail();
  ...
  //----------------------------------------------------------------------
  const std::vector<std::vector<art::Ptr<recob::OpHit>>> PhotonBackTracker::TrackIDsToOpHits(
  ...
  //----------------------------------------------------------------------
  double PhotonBackTracker::OpHitChargeCollectionPurity(
    std::set<int> trackIDs,
    std::vector<art::Ptr<recob::OpHit>> const& opHits)
  {
    shouldThisFail();
  ...
  //----------------------------------------------------------------------
  double PhotonBackTracker::OpHitChargeCollectionEfficiency(
    std::set<int> trackIDs,
    std::vector<art::Ptr<recob::OpHit>> const& opHits,
    std::vector<art::Ptr<recob::OpHit>> const& allOpHits,
    geo::View_t const& view)
  ...
  //----------------------------------------------------------------------
  const art::Ptr<sim::OpDetBacktrackerRecord> PhotonBackTracker::FindOpDetBacktrackerRecord(
    int opDetNum) const
  {
    shouldThisFail();
    ...
    for (size_t sc = 0; sc < cOpDetBacktrackerRecords.size(); ++sc) {
      ...
      if (cOpDetBacktrackerRecords[sc]->OpDetNum() == opDetNum)
        opDet = cOpDetBacktrackerRecords[sc];
    }
  ...
  //----------------------------------------------------------------------
  void PhotonBackTracker::OpHitToSDPs(recob::OpHit const& opHit, std::vector<sim::SDP>& sdps) const
  {
    shouldThisFail();
    // Get services.
    //const detinfo::DetectorClocks* ts = lar::providerFrom<detinfo::DetectorClocksService>();
