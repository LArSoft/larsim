...
bool TruthMatchUtils::isShowerParticle(
      const int trackID)
  {
    ...
    const std::string process = particle->Process();
    ...
    const bool isShowerElectron = (std::abs(pdgCode) == 11
                                    && (process == "ePair"    // pair production
                                     || process == "compt"    // compton scattering
                                     || process == "eIoni"    // ionisation
                                     || process == "phot"     // photoelectric effect
                                     || process == "conv"     // gamma conversion
                                    )
                                  );
    // photons in shower are generated by bremmstrahlung
    const bool isShowerPhoton = (pdgCode == 22
                                  && (process == "eBrem"    // bremsstrahlung
                                   || process == "annihil"  // positron annihilation
                                  )
                                );
...
TruthMatchUtils::G4ID TruthMatchUtils::FindShowerPrimaryID(
      const int trackID)
  {
      ...
      while (particle) {
        ...
        if (!TruthMatchUtils::isShowerParticle(parentID)) break; // parent is outside the shower, stop here
