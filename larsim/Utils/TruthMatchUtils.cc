...
bool TruthMatchUtils::isShowerParticle(const int trackID)
{
  ...
  const bool isShowerElectron =
    (std::abs(pdgCode) == 11 && (process == "ePair"    // pair production
  ...
  const bool isShowerPhoton = (pdgCode == 22 && (process == "eBrem"      // bremsstrahlung
...
TruthMatchUtils::G4ID TruthMatchUtils::FindShowerPrimaryID(const int trackID)
{
  art::ServiceHandle<cheat::ParticleInventoryService> piInv;
  const simb::MCParticle* particle = piInv->TrackIdToParticle_P(trackID);
  ...
  while (particle) {
    ...
    if (!TruthMatchUtils::isShowerParticle(parentID))
      break; // parent is outside the shower, stop here
                                                 ...
                                                 const bool rollupUnsavedIDs)
{
  ...
  for (const sim::TrackIDE& trackIDE : trackIDEs) {
    const G4ID g4ID(
      ...
      static_cast<G4ID>(rollupUnsavedIDs ?
