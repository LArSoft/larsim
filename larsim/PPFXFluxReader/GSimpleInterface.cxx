...
namespace fluxr {
  ...
  {
    if (!fFluxTree->GetEntry(ientry)) return false;

    fNuPos = TLorentzVector(
      fGSimpleEntry->vtxx * 100, fGSimpleEntry->vtxy * 100, fGSimpleEntry->vtxz * 100, 0);
    fNuMom =
      TLorentzVector(fGSimpleEntry->px, fGSimpleEntry->py, fGSimpleEntry->pz, fGSimpleEntry->E);
    ...
    if (fGSimpleAux && fGSimpleMeta) {
      ...
      for (size_t id = 0; id < auxdblname.size(); ++id) {
        ...
        if ("fgXYWgt" == auxdblname[id]) { flux.fnwtnear = flux.fnwtfar = auxdbl[id]; }
      }
