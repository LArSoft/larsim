void XProj()
{
  for (int integer =160; integer <250 ; integer= integer+10){ 
    //Double_t blue[]    = {0, 0, 1};
    //Double_t red[]     = {1, 0, 0};
    ...
    TFile *hybsmall = new TFile("hybrid_all_49.root");
    ...
    TH2D* hist1 = (TH2D*) hybsmall->Get(TString::Format("analyze/projZ%d", integer).Data());
    //TH2D* hist1 = (TH2D*) hyb->Get("analyze/ZProjection");
    
    TFile *hyb = new TFile("hybrid_d140_49.root");
    //TFile *std = new TFile("std_withoutFC_49.root"); 
    //TH2D* hist2 = (TH2D*) std->Get("analyze/projZ206");
    TH2D* hist2 = (TH2D*) hyb->Get(TString::Format("analyze/projZ%d", integer).Data());
    ...
    //hist1->Draw("colz");
    hist1->SetTitle(TString::Format("Hybrid Library projZ%d", integer).Data());
    ...
    hist2->SetTitle(TString::Format("Hibrid Library for distances d<140 cm, parametrization for d>140 cm projZ%d", integer).Data());
    ...
    hist3->SetTitle(TString::Format("Ratio of 1st / 2nd plot: exception points with d>140 for projZ%d OptDet #50", integer).Data());
    ...
    //c1->SetRightMargin(1.4);
    c1->Update();
    c1->SaveAs(TString::Format("plot/projZ%d #50.png", integer).Data());
