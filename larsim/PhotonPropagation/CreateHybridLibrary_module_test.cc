...
namespace phot
{
  ...
  { 
    ...
    for(unsigned int opdetIdx =90; opdetIdx <= geom->NOpDets(); ++opdetIdx){ //BEA
	...
	//g3.SetPoint(g3.GetN(), dist, chisq);
	  ...
	  g3.SetPoint(g3.GetN(), v.dist, chisq);
	  vox = v.vox;
          vis = v.vis;
          dist = v.dist;
          costheta = v.costheta;
      ...
      c1->Print("plots/Bea_vis_dist.pdf");
      
      g3.SetMarkerStyle(7);
      g3.GetXaxis()->SetTitle("Distance (cm)");
      g3.GetYaxis()->SetTitle("Chisquare");
      c1.cd(2);
      g3.Draw("AC");
