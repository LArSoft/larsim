void XProj()
{
  for (int integer =190; integer <200 ; integer++){ 
    Double_t blue[]    = {0, 0, 1};
    Double_t red[]     = {1, 0, 0};
    ...
    TH2D* hist1 = (TH2D*) hyb->Get(TString::Format("analyze/projZ%d", integer).Data());
    ...
    TH2D* hist2 = (TH2D*) std->Get(TString::Format("analyze/projZ%d", integer).Data());
    ...
    hist2->SetTitle(TString::Format("Std Library projZ%d", integer).Data());
    ...
    hist3->SetTitle(TString::Format("Ratio Hyb/Std projZ%d OptDet #50", integer).Data());
