...
namespace {
  ...
  //......................................................................
  double*
  interpolate(std::vector<double>& xData,
              ...
              bool extrapolate)
  {
    ...
    double yL1 = yData1[i], yR1 = yData1[i + 1], yL2 = yData2[i], yR2 = yData2[i + 1],
           yL3 = yData3[i], yR3 = yData3[i + 1];
  ...
  double
  Disk_SolidAngle(double* x, double* p)
  {
    const double d = x[0];
    const double h = x[1];
    const double b = p[0];
    ...
    if (d == 0) { return 2. * TMath::Pi() * (1. - aa); }
    const double bb = TMath::Sqrt(4 * b * d / (h * h + (b + d) * (b + d)));
    const double cc = 4 * b * d / ((b + d) * (b + d));

    if (TMath::Abs(boost::math::ellint_1(bb) - bb) < 1e-10 &&
        TMath::Abs(boost::math::ellint_3(cc, bb) - cc) < 1e-10) {
      throw(std::runtime_error("Problem loading ELLIPTIC INTEGRALS running Disk_SolidAngle!"));
    }
    if (d < b) {
      return 2. * TMath::Pi() -
             2. * aa *
               (boost::math::ellint_1(bb) + TMath::Sqrt(1. - cc) * boost::math::ellint_3(bb, cc));
    }
    if (d == b) { return TMath::Pi() - 2. * aa * boost::math::ellint_1(bb); }
    if (d > b) {
      return 2. * aa *
             (TMath::Sqrt(1. - cc) * boost::math::ellint_3(bb, cc) - boost::math::ellint_1(bb));
    }
    return 0.;
  }
  ...
  //......................................................................
  double
  Disk_SolidAngle(double d, double h, double b)
  {
    double x[2] = {d, h};
    double p[1] = {b};
  ...
  double
  Rectangle_SolidAngle(double a, double b, double d)
  {
    double aa = a / (2.0 * d);
    double bb = b / (2.0 * d);
  ...
  //......................................................................
  double
  Rectangle_SolidAngle(acc& out, TVector3 v)
  {
    ...
    if (v.Y() == 0.0 && v.Z() == 0.0) { return Rectangle_SolidAngle(out.h, out.w, v.X()); }
    ...
    if ((std::abs(v.Y()) > out.h / 2.0) && (std::abs(v.Z()) > out.w / 2.0)) {
      ...
      d = std::abs(v.X());
      double to_return =
        (Rectangle_SolidAngle(2 * (A + a), 2 * (B + b), d) -
         Rectangle_SolidAngle(2 * A, 2 * (B + b), d) - Rectangle_SolidAngle(2 * (A + a), 2 * B, d) +
         Rectangle_SolidAngle(2 * A, 2 * B, d)) /
        4.0;
      return to_return;
    }
    ...
    if ((std::abs(v.Y()) <= out.h / 2.0) && (std::abs(v.Z()) > out.w / 2.0)) {
      ...
      double to_return =
        (Rectangle_SolidAngle(2 * (a - A), 2 * (B + b), d) -
         Rectangle_SolidAngle(2 * (a - A), 2 * B, d) + Rectangle_SolidAngle(2 * A, 2 * (B + b), d) -
         Rectangle_SolidAngle(2 * A, 2 * B, d)) /
        4.0;
      return to_return;
    }
  ...
  //......................................................................
  double
  Gaisser_Hillas(double x, double* par)
  {
    double X_mu_0 = par[3];
  ...
  //......................................................................
  double
  Pol_5(double x, double* par)
  {
    // 5th order polynomial function
    return par[0] + par[1] * x + par[2] * pow(x, 2) + par[3] * pow(x, 3) + par[4] * pow(x, 4) +
           par[5] * pow(x, 5);
  }
} // namespace std
...
namespace phot {
  ...
  public:
    ...
    std::vector<double> propagationtime(G4ThreeVector x0,
                                        int OpChannel,
                                        int NPhotons,
                                        bool Reflected);
  ...
  PDFastSimPAR::produce(art::Event& event)
  {
    ...
    for (auto const& edepi : *edeps) {
      ...
      for (size_t channel = 0; channel < nOpChannels; channel++) {
        ...
        TVector3 OpDetPoint(
          fOpDetCenter.at(channel)[0], fOpDetCenter.at(channel)[1], fOpDetCenter.at(channel)[2]);
  ...
  PDFastSimPAR::Initialization()
  {
    ...
    for (size_t i = 0; i != pvs->NOpChannels(); i++) {
      ...
      std::cout << "OpChannel: " << i << "  Optical_Detector_Type: " << type_i
                ...
                << "  APERTURE_width: " << geo->OpDetGeoFromOpDet(i).Length() << std::endl;
    }
    ...
    if (pvs->IncludePropTime()) {
      ...
      int num_params =
        (fmax_d - 25) /
        fstep_size; // for d < 25cm, no parameterisaton, a delta function is used instead
                        ...
                        int optical_detector_type)
  {
    ...
    if (((ScintPoint[0] < 0) != (OpDetPoint[0] < 0)) && std::abs(OpDetPoint[0]) > 10) { return 0; }
    ...
    double distance =
      sqrt(pow(ScintPoint[0] - OpDetPoint[0], 2) + pow(ScintPoint[1] - OpDetPoint[1], 2) +
           pow(ScintPoint[2] - OpDetPoint[2], 2));
    ...
    double hits_geo =
      exp(-1. * distance / fL_abs_vuv) * (solid_angle / (4 * CLHEP::pi)) * Nphotons_created;
    ...
    double distance_to_corner =
      sqrt(y_to_corner * y_to_corner + z_to_corner * z_to_corner); // in the ph-cathode plane
    double pars_ini_[4] = {
    ...
    int hits_vuv =
      std::round(hits_rec); // round to integer value, cannot have non-integer number of hits
                        ...
                        int optical_detector_type)
  {
    ...
    double cathode_hits_geo = exp(-1. * distance_cathode / fL_abs_vuv) *
    ...
    if (optical_detector_type == 0) // rectangular aperture
    {
      ...
      detPoint.h = fydimension;
      detPoint.w =
        fzdimension; // width and height in cm of optical detector active window [rectangular aperture]
      // calculate solid angle
    ...
    double hits_geo =
      ...
      cathode_hits_rec; // 2*pi rather than 4*pi due to presence of reflective foils (vm2000)
    ...
    double distance_vuv =
      sqrt(pow(ScintPoint[0] - hotspot[0], 2) + pow(ScintPoint[1] - hotspot[1], 2) +
           pow(ScintPoint[2] - hotspot[2], 2));
    double distance_vis =
      sqrt(pow(hotspot[0] - OpDetPoint[0], 2) + pow(hotspot[1] - OpDetPoint[1], 2) +
           pow(hotspot[2] - OpDetPoint[2], 2));
    ...
    if (fApplyVisBorderCorrection) {
      ...
      if (fVisBorderCorrectionType == "Radial") {
        r = sqrt(pow(ScintPoint[1] - fcathode_ydimension, 2) +
                 pow(ScintPoint[2] - fcathode_zdimension, 2));
      }
      ...
      for (int i = 0; i < nbins_r; i++) {
        interp_vals[i] = interpolate(
          fvis_border_distances_x, fvis_border_correction[k][i], std::abs(ScintPoint[0]), false);
      }
      // interpolate in r
  ...
  //......................................................................
  std::vector<double>
  PDFastSimPAR::propagationtime(G4ThreeVector x0, int OpChannel, int NPhotons, bool Reflected)
  {
    ...
    if (pvs->IncludeParPropTime() && pvs->IncludePropTime()) {
      ...
      G4ThreeVector OpDetPoint(fOpDetCenter.at(OpChannel)[0] * CLHEP::cm,
  ...
  //......................................................................
  std::vector<double>
  PDFastSimPAR::getVUVTime(double distance, int number_photons)
  {
    // pre-allocate memory
  ...
  //......................................................................
  std::vector<double>
  PDFastSimPAR::getVISTime(TVector3 ScintPoint, TVector3 OpDetPoint, int number_photons)
  {
    // *************************************************************************************************
    ...
    // vuv part
    double vuv_time;
    if (VUVdist < 25) { vuv_time = VUVdist / fvuv_vgroup_mean; }
    else {
        ...
        do {
          ...
          if (counter >= 10) {
            ...
            arrival_time_smeared =
              arrival_time + (arrival_time - fastest_time) * (exp(-tau * log(x)) - 1);
          }
          // increment counter
          counter++;
  ...
  //......................................................................
  void
  PDFastSimPAR::generateparam(int index)
  {
    // get distance
    ...
    double* pars_landau = interpolate(
      fparameters[0], fparameters[2], fparameters[3], fparameters[1], distance_in_cm, true);
