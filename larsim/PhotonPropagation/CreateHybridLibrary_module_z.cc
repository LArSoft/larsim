...
namespace phot
{
  ...
  { 
	...
	if(opdetvec[2]-3 <v.ez && v.ez < opdetvec[2] + 3){
	  if(v.ey <opdetvec[1]+5.5 && v.ey >opdetvec[1]-5.5){
	  //std::cout << opdetIdx << " / " << geom->NOpDets() << "position center (z)"<<"  "<<opdetvec[2]<< std::endl;
	  g3.SetPoint(g3.GetN(), v.ex, obs*2e-5);
	  g4.SetPoint(g4.GetN(), v.ex, pred*2e-5);
	  //std::cout << opdetIdx << " / " << geom->NOpDets() << "position center (z)"<<"  "<<opdetvec[2]<< std::endl;
	  //std::cout << "Position in (x, y, z) =  " << v.ex << "     "<< v.ey << "     "<< v.ez << "  "<< std::endl;
