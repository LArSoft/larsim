...
namespace phot
{
  ...
  { 
    ...
    for(unsigned int opdetIdx =0; opdetIdx < 1; ++opdetIdx){//geom->NOpDets(); ++opdetIdx){
	...
	if(140-3.797 <v.ex && v.ex <= 140+3.797){
	  if(opdetvec[2]-2.911 <v.ez && v.ez <= opdetvec[2]+2.911){
	  //std::cout << opdetIdx << " / " << geom->NOpDets() << "position center (z)"<<"  "<<opdetvec[2]<< std::endl;
	    g3.SetPoint(g3.GetN(), v.ey, obs*2e-5);
	    g4.SetPoint(g4.GetN(), v.ey, pred*2e-5);
	    double x_vox = v.ex;
	    double z_vox = v.ez;
	  //std::cout << opdetIdx << " / " << geom->NOpDets() << "position center (z)"<<"  "<<opdetvec[2]<< std::endl;
	  //std::cout << opdetIdx <<" OpDet's position in (x, y, z) =  " << opdetvec[0] << "     "<< opdetvec[1] << "     "<< opdetvec[2]<< std::endl;
	  //std::cout << "Voxel's position in (x, y, z) =  " << v.ex << "     "<< v.ey << "     "<< v.ez << "  "<< std::endl;
	...
	h.Fill(chisq);
	  ...
	  if(140-3.797 <v.ex && v.ex <= 140+3.797){
	    if(opdetvec[2]-2.911 <v.ez && v.ez <= opdetvec[2]+2.911){
       	      g5.SetPoint(g5.GetN(), v.ey, obs*2e-5);
	    }
	  }
      ...
      g4.SetMarkerStyle(7);
      g5.SetMarkerStyle(3);
      g4.GetXaxis()->SetTitle("y-coordinate [cm]");
      ...
      c2->SaveAs(TString::Format("plots/Bea_vis_vs_y_%d.png", opdetIdx).Data());
