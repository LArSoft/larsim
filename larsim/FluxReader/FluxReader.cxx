...
namespace fluxr {
			    ...
			    art::EventPrincipal* &outE)
  {
    ...
    if (fSubRunID.runID() != newID.runID()) { // New Run
      ...
      mf::LogInfo(__FUNCTION__)<<"Cached run number ("<<fSubRunID.run()<< ") does not match current run number ("<<newID.run()<<")! Reassigning variable...."<<std::endl;
    }
    if (fSubRunID != newID) { // New SubRun
      mf::LogInfo(__FUNCTION__)<<"Cached subrun number ("<<fSubRunID.subRun()<< ") does not match current subrun number ("<<newID.subRun()<<")! Reassigning variable...."<<std::endl;
