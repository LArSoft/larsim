...
void print_weights( const std::string& filename ) {
  ...
  for ( ev.toBegin(); !ev.atEnd(); ++ev ) {
    ...
    auto mctruth_handle = ev.getValidHandle<
      std::vector<simb::MCTruth> >( genie_producer_label );
    auto gtruth_handle = ev.getValidHandle<
      std::vector<simb::GTruth> >( genie_producer_label );

    // Use the EventWeight producer label here
    auto weights_handle = ev.getValidHandle<
      std::vector<evwgh::MCEventWeight> >( eventweight_producer_label );
    ...
    for ( size_t v = 0u; v < mctruth_handle->size(); ++v ) {
      ...
      if ( nu.Nu().NumberTrajectoryPoints() > 0 ) {
        ...
        else if ( mode == 1 ) std::cout << "RES";
        else if ( mode == 2 ) std::cout << "DIS";
        else if ( mode == 3 ) std::cout << "COH";
        else if ( mode == 10 ) std::cout << "MEC";
        else std::cout << " other";
        std::cout << " interaction with neutrino energy = " <<
          E_nu << " GeV\n";
      }
