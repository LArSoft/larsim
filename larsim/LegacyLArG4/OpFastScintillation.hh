...
namespace larg4 {
  ...
  public:
    ...
    // OpFastScintillation Process has both PostStepDoIt (for energy deposition of
    // particles in flight) and AtRestDoIt (for energy given to the medium by particles at
    // rest)
    ...
    // 'StronglyForced' condition for the DoIt to be invoked at every step.

    G4double GetMeanLifeTime(const G4Track& aTrack, G4ForceCondition*);
    // Returns infinity; i. e. the process does not limit the time, but sets the
    // 'StronglyForced' condition for the DoIt to be invoked at every step.
    ...
    // If set, the primary particle tracking is interrupted and any produced scintillation
    // photons are tracked next. When all have been tracked, the tracking of the primary
    // resumes.

    void SetFiniteRiseTime(const G4bool state);
    // If set, the OpFastScintillation process expects the user to have set the constant
    // material property FAST/SLOWSCINTILLATIONRISETIME.
    ...
    // Called to set the scintillation photon yield factor, needed when the yield is
    // different for different types of particles. This scales the yield obtained from the
    ...
    // Called to set the scintillation exciation ratio, needed when the scintillation
    // level excitation is different for different types of particles. This overwrites the
    ...
    // Return the boolean that determines the method of scintillation production
    ...
    // Functions for vuv component Landau + Exponential timing parameterisation, updated
    // method
  ...
  protected:
    ...
    // It builds either the fast or slow scintillation integral table; or both.

    bool RecordPhotonsProduced(const G4Step& aStep, double N);
    // Note the production of N photons in at point xyz.  pass on to generate detector
    // response, etc.
  ...
  inline double dist(const TReal* x, const TReal* y, const unsigned int dimension)
  {
    double d = 0.;
  ...
  inline double dist(const std::array<double, 3> x,
                     const TVector3 y,
