...
namespace larg4 {
  ...
  AuxDetReadout::AuxDetReadout(std::string const& name, unsigned int adNum, unsigned int svNum)
    : G4VSensitiveDetector(name), fAuxDet(adNum), fAuxDetSensitive(svNum)
  {}
  ...
  G4bool AuxDetReadout::ProcessHits(G4Step* step, G4TouchableHistory*)
  {
    ...
    double startWorld[3] = {
      startG4.getX() / CLHEP::cm, startG4.getY() / CLHEP::cm, startG4.getZ() / CLHEP::cm};

    double startTime = step->GetPreStepPoint()->GetGlobalTime() / CLHEP::ns;

    G4ThreeVector stopG4(step->GetPostStepPoint()->GetPosition());
    double stopWorld[3] = {
      stopG4.getX() / CLHEP::cm, stopG4.getY() / CLHEP::cm, stopG4.getZ() / CLHEP::cm};
  ...
  void AuxDetReadout::AddParticleStep(int inputTrackID,
