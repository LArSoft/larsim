...
//
// This class is a physics process based on the standard Geant4 scintillation process.
//
// It has been stripped down and adapted to form the backbone of the LArG4 fast optical
// simulation.  Photons, instead of being produced and added to the geant4 particle stack,
// are logged and used to predict the visibility of this step to each PMT in the detector.
//
// The photonvisibilityservice looks up the visibility of the relevant xyz point, and if a
// photon is detected at a given PMT, one OnePhoton object is logged in the
// OpDetPhotonTable
//
// At the end of the event, the OpDetPhotonTable is read out by LArG4, and detected
// photons are stored in the event.
//
// This process can be used alongside the standard Cerenkov process, which does step
// geant4 opticalphotons.  Both the fast scintillation table and the geant4 sensitive
// detectors are read out by LArG4 to produce a combined SimPhoton collection.
//
// Added disclaimer : This code is gross.  Thats basically because it adheres to the
//  original, gross Geant4 implementation.
...
namespace larg4 {
    ...
    , fOnlyActiveVolume(usesSemiAnalyticModel())
  {
    ...
    if (bPropagate) {
      ...
      // Loading the position of each optical channel, neccessary for the
      // parametrizatiuons of Nhits and prop-time
      ...
      if (fPVS->IncludePropTime()) {
        ...
        // that are generated as they are required default TF1() constructor gives
        // function with 0 dimensions, can then check numDim to qucikly see if a
        // parameterisation has been generated
  ...
  // This routine simply calls the equivalent PostStepDoIt since all the necessary
  // information resides in aStep.GetTotalEnergyDeposit()
  {
  ...
  // scintillator. A Poisson/Gauss-distributed number of photons is generated according to
  // the scintillation yield formula, distributed evenly along the track segment and
  // uniformly into 4pi.
  {
    aParticleChange.Initialize(aTrack);
    // Check that we are in a material with a properties table, if not just return
    ...
    // Retrieve the Scintillation Integral for this material new
    // G4PhysicsOrderedFreeVector allocated to hold CII's
    ...
    //
    // The ratio of slow photons to fast photons is related by the yieldratio parameter.
    //  G4's poisson fluctuating scheme is a bit different to ours - we should check that
    //  they are equivalent.
    //
    // G4 poisson fluctuates the number of initial photons then divides them with a
    // constant factor between fast + slow, whereas we poisson fluctuate separateyly the
    // fast and slow detection numbers.
    //

    // get the number of photons produced from the IonizationAndScintillation singleton
  ...
  bool OpFastScintillation::RecordPhotonsProduced(const G4Step& aStep, double MeanNumberOfPhotons)
  {
    ...
    if (scintillationByParticleType) {
      // The scintillation response is a function of the energy deposited by particle
      // types.

      // Get the definition of the current particle
      G4ParticleDefinition* pDef = aParticle->GetDefinition();

      // Obtain the G4MaterialPropertyVectory containing the scintillation light yield as
      // a function of the deposited energy for the current particle type
      ...
      // Electrons (must also account for shell-binding energy attributed to gamma from
      // standard PhotoElectricEffect)
      ...
      // If the user has not specified yields for (p,d,t,a,carbon) then these unspecified
      // particles will default to the electron's scintillation yield
      if (YieldRatio == 0) {
        YieldRatio = aMaterialPropertiesTable->GetConstProperty("ELECTRONYIELDRATIO");
      }
    }
    ...
    for (G4int scnt = 1; scnt <= nscnt; scnt++) {
      ...
      for (size_t Reflected = 0; Reflected <= 1; ++Reflected) {
        ...
        for (auto itdetphot = itstart; itdetphot != itend; ++itdetphot) {
          ...
          double Edeposited = 0;
          if (scintillationByParticleType) {
            // We use this when it is the only sensical information. It may be of limited
            // use to end users.
          ...
          // We need to split the energy up by the number of photons so that we never try
          // to write a 0 energy.
  ...
  void OpFastScintillation::BuildThePhysicsTable()
  {
    ...
    for (G4int i = 0; i < numOfMaterials; i++) {
      ...
      // Retrieve vector of scintillation wavelength intensity for the material from the
      // material's optical properties table.
      ...
      if (aMaterialPropertiesTable) {
        ...
        if (theFastLightVector) {
          // Retrieve the first intensity point in vector of (photon energy, intensity)
          // pairs
      ...
      // The scintillation integral(s) for a given material will be inserted in the
      // table(s) according to the position of the material in the material table.
      theFastIntegralTable->insertAt(i, aPhysicsOrderedFreeVector);
      theSlowIntegralTable->insertAt(i, bPhysicsOrderedFreeVector);
    }
  }

  // Called by the user to set the scintillation yield as a function of energy deposited
  // by particle type
                                            ...
                                            bool Reflected)
  {
    ...
    if (fPVS->IncludeParPropTime() && fPVS->IncludePropTime()) {
      ...
      // Warning: TF1::GetNdim()==1 will tell us if the TF1 is really defined or it is the
      // default one.  This will fix a segfault when using timing and interpolation.
  ...
  G4double OpFastScintillation::sample_time(const G4double tau1, const G4double tau2) const
  {
    ...
    while (1) {
      ...
      // make sure the envelope function is always larger than the bi-exponential
  ...
  // New Parametrization code parameterisation generation function
  void OpFastScintillation::generateParam(const size_t index, const size_t angle_bin)
  {
    ...
    // Defining the model function(s) describing the photon transportation timing vs
    // distance Getting the landau parameters from the time parametrization
    ...
    interpolate3(pars_landau,
                 ...
                 distance_in_cm,
                 true);
    // Deciding which time model to use (depends on the distance) defining useful times
    // for the VUV arrival time shapes
    ...
    // set the number of points used to sample parameterisation for shorter distances,
    // peak is sharper so more sensitive sampling required
    ...
    // calculate max and min distance relevant to sample parameterisation max
    ...
    // store TF1 and min/max, this allows identical TF1 to be used every time sampling the
    // first call of GetRandom generates the timing sampling and stores it in the TF1
    // object, this is the slow part all subsequent calls check if it has been generated
    // previously and are ~100+ times quicker
                                        ...
                                        const TVector3& OpDetPoint)
  {
    ...
    // 2). cutoff = largest smeared time allowed, preventing excessively large times
    //     caused by exponential distance to cathode
    ...
    for (size_t i = 0; i < arrivalTimes.size(); ++i) {
      ...
      // otherwise smear
        ...
        // loop until time generated is within cutoff limit; most are within single
        // attempt, very few take more than two
        do {
  ...
  void OpFastScintillation::detectedReflecHits(std::map<size_t, int>& ReflDetectedNum,
                                               ...
                                               geo::Point_t const& ScintPoint) const
  {
    // 1). calculate total number of hits of VUV photons on reflective foils via solid
    //     angle + Gaisser-Hillas corrections:
    ...
    // calculate distance and angle between ScintPoint and hotspot; vast majority of hits
    // in hotspot region directly infront of scintpoint, therefore consider attenuation
    // for this distance and on axis GH instead of for the centre coordinate
    ...
    // determine Gaisser-Hillas correction including border effects use flat correction
                                   ...
                                   OpticalDetector const& opDet) const
  {
    ...
    // calculate number of photons hits by geometric acceptance: accounting for solid
    // angle and LAr absorption length
    double hits_geo =
      std::exp(-1. * distance / fL_abs_vuv) * (solid_angle / (4 * CLHEP::pi)) * Nphotons_created;

    // apply Gaisser-Hillas correction for Rayleigh scattering distance and angular
    // dependence offset angle bin
    const size_t j = (theta / fdelta_angulo_vuv);

    // determine GH parameters, accounting for border effects radial distance from centre
    // of detector (Y-Z)
  ...
  {
    // 1). calculate total number of hits of VUV photons on reflective foils via solid
    //     angle + Gaisser-Hillas corrections.  Done outside as it doesn't depend on
    //     OpDetPoint
    ...
    // 2). calculate number of these hits which reach the optical detector from the
    //     hotspot via solid angle
    ...
    // calculate distances and angles for application of corrections; distance from
    // hotspot to optical detector
                                             ...
                                             geo::Point_t const& OpDetPoint) const
  {
    ...
    // temporary method working for SBND, uBooNE, DUNE 1x2x6; to be replaced to work in
    // full DUNE geometry; check x coordinate has same sign or is close to zero, otherwise
    // return 0 hits
  ...
  //======================================================================
  // Returns interpolated value at x from parallel arrays ( xData, yData ) Assumes that
  // xData has at least two elements, is sorted and is strictly monotonic increasing
  ...
  // TODO: allow greater tolerance in comparisons, by default it's using:
  // std::numeric_limits<double>::epsilon(): 2.22045e-16 that's an unrealistic small
  // number, better setting constexpr double tolerance = 0.0000001; // 1 nm
  ...
  {
    // v is the position of the track segment with respect to the center position of the
    // arapuca window
  ...
  {
    // this function calculates the solid angle of a semi-sphere of radius b, as a
    // correction to the analytic formula of the on-axix solid angle, as we move off-axis
    // an angle theta. We have used 9-angular bins with delta_theta width.
