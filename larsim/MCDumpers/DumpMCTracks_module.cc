...
namespace {
  ...
  struct Config {
    ...
    fhicl::Atom<art::InputTag> InputTracks{Name("InputTracks"),
                                    ...
                                    ) const
{
  ...
  out << "from GEANT track ID=" << track.TrackID() << " PDG ID=" << track.PdgCode() << " from "
      << OriginDescription(track.Origin()) << " via '" << track.Process() << "'";
  out << "\n" << indent << "  starting at ";
  ...
  out << "\n" << indent;
  if (nSteps > 0) {
    out << "energy information for " << nSteps << " steps (dE/dX in MeV/cm, then dQ/dx per plane):";
  ...
  out << "\n"
      << indent << "mother ID=" << track.MotherTrackID() << " PDG ID=" << track.MotherPdgCode()
  ...
  out << "\n"
      << indent << "ancestor ID=" << track.AncestorTrackID()
      << " PDG ID=" << track.AncestorPdgCode() << " via '" << track.AncestorProcess() << "'";
  out << "\n" << indent << "  starting at ";
...
void sim::DumpMCTracks::analyze(art::Event const& event)
{
  ...
  auto const& Tracks = *(event.getValidHandle<std::vector<sim::MCTrack>>(fInputTracks));

  mf::LogVerbatim(fOutputCategory)
    << "Event " << event.id() << ": data product '" << fInputTracks.encode() << "' contains "
