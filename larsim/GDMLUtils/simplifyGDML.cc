...
std::string const ConfigurationParameters::DefaultSchemaURL
...
void ConfigurationParser::printHelp
  (int exitCode /* = 0 */, const char* progName /* = "simplifyGDML" */)
{
  std::cout <<
      "Asks GEANT4 to process and rewrite a GDML file."
...
ConfigurationParser::error ConfigurationParser::parse
  (ConfigurationParameters& params, unsigned int argc, char** argv)
{
    ...
    ((ch = getopt_long(argc, argv, shortopts.c_str(), longopts, NULL)) != -1)
  {
    switch (ch) {
      case opt::Validate:
        ...
        continue;
      case opt::Setup:
        params.setupName = optarg;
        continue;
      case opt::Schema:
        params.schemaPath = optarg;
        continue;
      case opt::Overwrite:
        params.overwrite = true;
        continue;
      case opt::NoWrite:
        params.dontWrite = true;
        continue;
      case opt::Debug:
        if (optarg) {
        ...
        else params.debugLevel = DefaultDebugLevel;
        continue;
      case '?': // this is special since has a specific meaning for getopt()
        if (optopt != '?') {
          if (!forgiving) {
            std::cerr
              << "Invalid option: '" << argv[optind-1] << "'" << std::endl;
          }
      ...
      case opt::Help:
        ...
        continue;
      default:
        std::cerr << "Internal error: option '" << ch << "' not supported yet."
          << std::endl;
...
std::string addNameSuffix(std::string name, std::string suffix) {
  ...
  if (iExt == std::string::npos) name.append(suffix);
  else name.insert(iExt, suffix);
  return name;
} // addNameSuffix()
...
//------------------------------------------------------------------------------
G4VPhysicalVolume* readWorldVolume
  (G4GDMLParser& parser, ConfigurationParameters const& params)
{
  decltype(auto) worldVolume = parser.GetWorldVolume(params.setupName);
  if (!worldVolume) {
    std::cerr << "Failed to find the world volume for setup '"
...
int writeWorld(G4GDMLParser& parser, ConfigurationParameters const& params) {
  ...
  if (exists(params.destPath)) {
    if (params.overwrite) {
      ...
      std::cerr << "Destination file '" << params.destPath
