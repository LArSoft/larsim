...
namespace detsim {
  ...
  void SimWireAna::beginJob()
  {
    ...
    fCompressErr2D =
      tfs->make<TH2F>("compressErr2D", ";Raw;Raw-Compressed", 100, -50., 50., 1000, -495.5, 500.5);
    fRawVsCompress =
      tfs->make<TH2F>("rawVsCompress", ";Raw;Compressed", 100, -50., 50., 100, -50., 50.);
    ...
    for (unsigned int rd = 0; rd < rdvec.size(); ++rd) {
      ...
      if (uncompressed.size() != rdvec[rd]->Samples()) {
        cet::exception("WrongSizeUncompress")
          ...
          << "original = " << rdvec[rd]->Samples() << " uncompress = " << uncompressed.size()
          << "\n";
      }
      ...
      for (unsigned int t = 0; t < uncompressed.size(); ++t) {
        ...
        if (uncompressed[t] - rdvec[rd]->ADC(t) > 1)
          mf::LogWarning("SimWireAna")
            << "problem with event "
            << " time " << t << " ADC " << rdvec[rd]->ADC(t) << " uncompress " << uncompressed[t]
