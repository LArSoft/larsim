  ...
  , _cosmic_distribution{p.get<bool>("CosmicDistribution", false)}
{
  ...
  for (size_t idx = 0; idx < minmult_v.size(); ++idx) {
    ...
    if (minmult_v[idx] > _multi_min) _multi_min = minmult_v[idx];
  }
                                    ...
                                    const double z)
{
  ...
  */
  ...
  if (!param.direct_inward) {
    ...
    if (_debug) {
      std::cout << "Generating XYZ direction sign for a particle at (" << x << "," << y << "," << z
                << ")" << std::endl
...
void MultiPartRain::produce(art::Event& e)
{
  ...
  if (_debug) std::cout << "Generating" << param_idx_v.size() << " particles..." << std::endl;
