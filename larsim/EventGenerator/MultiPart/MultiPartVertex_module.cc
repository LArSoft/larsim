...
public:
  ...
  std::array<double, 3U> GenBeamDirection(const TVector3& start_pos,
  ...
  TVector3 GenMomentum(const PartGenParam& param, const double& mass,
                       bool& same_range, const std::array<double, 3>& dir);
...
MultiPartVertex::MultiPartVertex(fhicl::ParameterSet const& p)
  ...
  , fFlatEngine(art::ServiceHandle<rndm::NuRandomService>()->registerAndSeedEngine(
      createEngine(0, "HepJamesRandom", "GenVertex"),
      "HepJamesRandom",
      "GenVertex"))
// Initialize member data here.
{
  ...
  _revert = p.get<bool>("Revert", false);
  _use_boost = p.get<bool>("UseBoost", false);
  ...
  if (_beam_target_radius < 0.0)
    this->abort("BeamTargetRadius must be >= 0");
  ...
  if (beam_inward_v.size() != 3)
    this->abort("BeamInwardDirection must have exactly 3 elements");
  double norm = std::sqrt(beam_inward_v[0]*beam_inward_v[0] +
                          beam_inward_v[1]*beam_inward_v[1] +
  ...
  if (_beam_radius <= 0.)
    this->abort("BeamRadius must be positive!");

  if (_beam_mode && _debug > 0) {
    std::cout << "[MultiPartVertex] BeamMode ENABLED\n"
              << "  Entrance  : (" << _beam_entrance[0] << ", "
                                   << _beam_entrance[1] << ", "
                                   << _beam_entrance[2] << ") cm\n"
              << "  Radius    : " << _beam_radius << " cm\n"
              << "  Inward dir: (" << _beam_inward_dir[0] << ", "
                                   << _beam_inward_dir[1] << ", "
                                   << _beam_inward_dir[2] << ")\n"
...
// sanity check
  ...
  for (auto const& tpc_id : _tpc_v) {
    ...
    if (_debug) {
      std::cout << "Using Cryostat " << tpc_id[0] << " TPC " << tpc_id[1] << " ... X " << xmin
                << " => " << xmax << " ... Y " << ymin << " => " << ymax << " ... Z " << zmin
                << " => " << zmax << std::endl;
    }
  }
...
TVector3 MultiPartVertex::GenBeamEntrancePosition()
{
  ...
  TVector3 offset = _beam_radius * (st * TMath::Cos(phi) * e1 +
                                    st * TMath::Sin(phi) * e2 +
                                    ct               * n_up);

  TVector3 pos(_beam_entrance[0] + offset.X(),
               _beam_entrance[1] + offset.Y(),
               _beam_entrance[2] + offset.Z());

  if (_debug > 0)
    std::cout << "[BeamMode] Start position: ("
              << pos.X() << ", " << pos.Y() << ", " << pos.Z() << ") cm\n"
              << "           Offset from entrance: ("
              << offset.X() << ", " << offset.Y() << ", " << offset.Z() << ") cm" << std::endl;
...
TVector3 MultiPartVertex::GenBeamTarget() const
{
  ...
  TVector3 target(_beam_entrance[0] + offset.X(),
                  _beam_entrance[1] + offset.Y(),
                  _beam_entrance[2] + offset.Z());

  if (_debug > 0)
    std::cout << "[BeamMode] Target point: ("
              << target.X() << ", " << target.Y() << ", " << target.Z() << ") cm\n"
              << "           Offset from entrance: ("
              << offset.X() << ", " << offset.Y() << ", " << offset.Z() << ") cm" << std::endl;
...
std::array<double, 3U>
MultiPartVertex::GenBeamDirection(const TVector3& start_pos, const TVector3& target) const
{
  TVector3 dir = (target - start_pos).Unit();

  if (_debug > 0)
    std::cout << "[BeamMode] Direction: ("
...
TVector3 MultiPartVertex::GenBoost()
{
  ...
  if (_debug > 0) {
    std::cout << "Generated boost parameter gamma beta = " << gbmag << ", cos(theta) = " << ct
              << ", phi = " << phi << std::endl;
    std::cout << "Generating a boost (" << bx << "," << by << "," << bz << ")" << std::endl;
  }
                                      ...
                                      bool& same_range)
{
  ...
  double px, py, pz;
  if (!same_range) {
    TVector3 momentum = GenMomentum(param, mass);
    px = momentum.X();
    py = momentum.Y();
    pz = momentum.Z();
  }
...
{
  double p_sf =
    sqrt(sf * (2 * (sf - 1) * cet::square(m) -
               2 * (sf - 1) * m * sqrt(cet::square(m) + cet::square(p)) + sf * cet::square(p))) /
    p;
...
void MultiPartVertex::produce(art::Event& e)
{
  ...
  TVector3 pos_boost = GenBoost();
  double bx = pos_boost.X();
  double by = pos_boost.Y();
  double bz = pos_boost.Z();
  ...
  if (_revert) {
    ...
    for (size_t idx = 0; idx < E_vec.size(); ++idx) {
      ...
      if (_debug)
        std::cout << "E: " << E_vec[idx] << " , m: " << mass_vec[idx]
                  << " , KE: " << E_vec[idx] - mass_vec[idx] << std::endl;
    }
    ...
    if (total_KE > 1.) {
      for (size_t idx = 0; idx < param_idx_v.size(); ++idx) {
        ...
        if (param.kerange[1] != 1) {
          ...
          double temp_p =
            ...
            if (_debug)
            std::cout << "KE range: " << param.kerange[1] << " , pdg :  " << pdg_vec[idx]
                      << std::endl;
          mom_sf = GenMomentumSF(param.kerange[1], mass_vec[idx], temp_p);
          if (_debug)
            std::cout << "particle KE sf: " << param.kerange[1] << ", p : " << temp_p
                      << " , mass :  " << mass_vec[idx] << " , mom_sf: " << mom_sf << std::endl;
          double E_scaled = mass_vec[idx] + param.kerange[1] * (E_vec[idx] - mass_vec[idx]);
          if (_debug)
            std::cout << "particle E : " << E_vec[idx] << " , scaled E : " << E_scaled << std::endl;
          TLorentzVector mom(
            mom_sf * px_vec[idx], mom_sf * py_vec[idx], mom_sf * pz_vec[idx], E_scaled);
          if (_use_boost) mom.Boost(bx, by, bz);
          simb::MCParticle part(part_v.size(), pdg_vec[idx], "primary", 0, mass_vec[idx], 1);
          part.AddTrajectoryPoint(pos, mom);
          part_v.emplace_back(std::move(part));
      ...
      if (_debug)
        std::cout << "gen_total_KE: " << gen_total_KE << " , KE scale factor :  " << total_KE_sf
                  << std::endl;
      ...
      for (size_t idx = 0; idx < param_idx_v.size(); ++idx) {
        ...
        if (_debug)
          std::cout << "KE range: " << param.kerange[1] << " , pdg :  " << pdg_vec[idx]
                    << std::endl;
        mom_sf = GenMomentumSF(total_KE_sf * param.kerange[1], mass_vec[idx], temp_p);
        if (_debug)
          std::cout << "particle KE sf: " << total_KE_sf * param.kerange[1] << ", p : " << temp_p
                    << " , mass :  " << mass_vec[idx] << " , mom_sf: " << mom_sf << std::endl;
        double E_scaled =
          mass_vec[idx] + total_KE_sf * param.kerange[1] * (E_vec[idx] - mass_vec[idx]);
        if (_debug)
          std::cout << "particle E : " << E_vec[idx] << " , scaled E : " << E_scaled << std::endl;
        TLorentzVector mom(
          mom_sf * px_vec[idx], mom_sf * py_vec[idx], mom_sf * pz_vec[idx], E_scaled);
