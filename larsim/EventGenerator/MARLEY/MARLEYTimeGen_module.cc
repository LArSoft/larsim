...
namespace {
  ...
  // Convert GeV to MeV
  constexpr double GeV_to_MeV = 1e3;
  ...
  {
    return std::find(std::begin(NU_X_PDG_CODES), std::end(NU_X_PDG_CODES), pdg_code) !=
           std::end(NU_X_PDG_CODES);
  }
  ...
  double flux_averaged_total_xs(marley::NeutrinoSource& nu_source, marley::Generator& gen);
  ...
  // [x_min, x_max]
  double integrate(const std::function<double(double)>& f, double x_min, double x_max);
}
...
public:
  ...
  struct KeysToIgnore {
    std::set<std::string> operator()() { return {"marley_parameters"}; }
  };
  ...
  struct Config {
    ...
    fhicl::OptionalAtom<std::string> pinching_parameter_type_{
      ...
      [this]() -> bool {
        auto spectrum_file_format = marley_utils::to_lowercase(spectrum_file_format_());
...
protected:
  ...
  public:
    ...
    {
      return marley::IteratorToMember<It, double>(it, &FitParameters::fLuminosity);
    }
  ...
  protected:
    ...
    {
      if (pdg_code == marley_utils::ELECTRON_NEUTRINO) { return &TimeFit::fNueFitParams; }
  ...
  public:
    ...
    /// converted
    template <typename It>
    static marley::IteratorToMember<It, FitParameters> make_FitParameters_iterator(int pdg_code,
                                                                                   It iterator)
    {
      return marley::IteratorToMember<It, FitParameters>(iterator,
                                                         GetFitParametersMemberPointer(pdg_code));
    }
  ...
  simb::MCTruth make_uniform_energy_mctruth(double E_min,
                                            double E_max,
                                            double& E_nu,
  ...
  /// @brief The TTree to use to dump raw MARLEY events
  ...
  /// generated simb::MCTruth objects, but in MARLEY's native format
  TTree* fEventTree;
  ...
  , fFluxAveragedCrossSection(0.)
{
  ...
  // event objects (useful for debugging purposes). The actual branch for
  // storing the events is managed by the MARLEYHelper class.
                                              ...
                                              const TLorentzVector& vertex_pos)
{
  ...
  if (fSamplingMode == TimeGenSamplingMode::HISTOGRAM) {
    ...
    double E_nu = mc_truth.GetNeutrino().Nu().E() * GeV_to_MeV;
...
//------------------------------------------------------------------------------
std::unique_ptr<marley::NeutrinoSource> evgen::MarleyTimeGen::source_from_time_fit(
  const TimeFit& fit)
{
                                                                ...
                                                                const TLorentzVector& vertex_pos)
{
  ...
  auto mc_truth = fMarleyHelper->create_MCTruth(vertex_pos);
...
namespace {
                                ...
                                double& tot_xs_integ)
  {
    ...
    source_integ = integrate([&nu_source](double E_nu) -> double { return nu_source.pdf(E_nu); },
                             nu_source.get_Emin(),
                             nu_source.get_Emax());
